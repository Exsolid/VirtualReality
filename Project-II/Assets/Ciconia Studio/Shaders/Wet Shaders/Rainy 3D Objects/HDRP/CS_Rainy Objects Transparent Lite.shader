Shader "Ciconia Studio/CS_Wet Animation/HDRP/Rainy 3D Objects/Transparent/Lite"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _NormalDrop;
			float _ContrastTransparentMap1;
			float _Brightness;
			float _UseBaseColorAlpha;
			float _TrailDropContrast;
			float _SpreadTransparentMap1;
			float _OpacityGrabPass;
			float _EmissionIntensity;
			float _BlurReflection;
			float _ReflectionIntensity;
			float _Glossiness;
			float _SmoothnessDrop;
			float _InvertTransparent1;
			...
			float4 CalculateContrast( float contrastValue, float4 colorTarget )
			{
				...
				return mul( float4x4( contrastValue,0,0,t, 0,contrastValue,0,t, 0,0,contrastValue,t, 0,0,0,1 ), colorTarget );
					...
					float2 voronoihash58_g999( float2 p )
					{
					...
					float voronoi58_g999( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash58_g999( n + g );
			...
			float snoise( float2 v )
			{
				...
				return 130.0 * dot( m, g );
					...
					float2 voronoihash48_g1089( float2 p )
					{
					...
					float voronoi48_g1089( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash48_g1089( n + g );
								o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
			...
			float3 PerturbNormal107_g1092( float3 surf_pos, float3 surf_norm, float height, float scale )
			{
						...
						)
			{
				...
				float3 surf_pos107_g1092 = ase_worldPos;
				float3 surf_norm107_g1092 = normalWS;
				float2 temp_cast_3 = (_SplashSpeed).xx;
				float time58_g999 = ( 1.0 * 0.001 );
				float2 voronoiSmoothId58_g999 = 0;
				float2 texCoord55_g999 = packedInput.ase_texcoord5.xyz.xy * float2( 1,1 ) + float2( 0,0 );
				...
				float2 coords58_g999 = ( float4( texCoord55_g999, 0.0 , 0.0 ) + temp_cast_5 ).xy * _Tiling;
				float2 id58_g999 = 0;
				float2 uv58_g999 = 0;
				float fade58_g999 = 0.5;
				float voroi58_g999 = 0;
				float rest58_g999 = 0;
				for( int it58_g999 = 0; it58_g999 <2; it58_g999++ ){
				voroi58_g999 += fade58_g999 * voronoi58_g999( coords58_g999, time58_g999, id58_g999, uv58_g999, 0,voronoiSmoothId58_g999 );
				rest58_g999 += fade58_g999;
				coords58_g999 *= 2;
				fade58_g999 *= 0.5;
				}//Voronoi58_g999
				voroi58_g999 /= rest58_g999;
				float2 myVarName60_g999 = id58_g999;
				float2 panner63_g999 = ( 1.0 * _Time.y * temp_cast_3 + ( step( voroi58_g999 , 0.1 ) * myVarName60_g999 ));
				float temp_output_79_0_g999 = (0.0 + (_Size - 0.0) * (0.05 - 0.0) / (1.0 - 0.0));
				float temp_output_120_0_g999 = saturate( ( step( voroi58_g999 , temp_output_79_0_g999 ) * ( 1.0 - ( voroi58_g999 * temp_output_79_0_g999 * (1.0 + (_DotFalloff - 0.0) * (500.0 - 1.0) / (1.0 - 0.0)) ) ) ) );
				...
				float clampResult605 = clamp( ( ( saturate( ( tex2D( _GradientTexRain1, panner63_g999 ).g * temp_output_120_0_g999 ) ) * _Intensity ) * (( _EnableDropSide )?( ( saturate( max( blendOpSrc603, blendOpDest603 ) )) ):( (( _UseBaseColorAlphaMaskTop )?( ( temp_output_667_0 * BaseColorAlpha46 ) ):( temp_output_667_0 )) )) ) , 0.0 , 1.0 );
				float RainDrops606 = clampResult605;
				float4 temp_cast_8 = (RainDrops606).xxxx;
				float clampResult243_g1089 = clamp( _DripLineContrast , 0.0 , 100.0 );
				float3 worldToObj249_g1089 = mul( GetWorldToObjectMatrix(), float4( GetCameraRelativePositionWS(( mul( UNITY_MATRIX_M, float4( ase_worldPos , 0.0 ) ).xyz + float3( float2( 0.5,0.5 ) ,  0.0 ) )), 1 ) ).xyz;
				float4 appendResult16_g1089 = (float4(worldToObj249_g1089.z , worldToObj249_g1089.y , 0.0 , 0.0));
				float clampResult5_g1089 = clamp( _Amount , 8.0 , 1000.0 );
				float2 _Globaltiling = float2(1,1);
				float4 XProjectionAmount23_g1089 = ( appendResult16_g1089 * ( clampResult5_g1089 * ( _Globaltiling.y * _TilingHorizontal ) ) );
				float RandomPosition25_g1089 = _RandomPosition;
				float simplePerlin2D46_g1089 = snoise( ( floor( XProjectionAmount23_g1089 ) * float4( (( _EnableProjectionGrid )?( float2( 1,1 ) ):( float2( 1,0 ) )), 0.0 , 0.0 ) ).xy*RandomPosition25_g1089 );
				simplePerlin2D46_g1089 = simplePerlin2D46_g1089*0.5 + 0.5;
				float temp_output_240_0_g1089 = (1.0 + (_GrowMask - 1.0) * (50.0 - 1.0) / (10.0 - 1.0));
				float temp_output_83_0_g1089 = round( ( pow( simplePerlin2D46_g1089 , 2.0 ) * temp_output_240_0_g1089 ) );
				float RF_RandomMin66_g1089 = 0.05;
				float RF_Random65_g1089 = (0.05 + (_RandomMax - 0.1) * (1.0 - 0.05) / (1.0 - 0.1));
				float2 appendResult88_g1089 = (float2(RF_RandomMin66_g1089 , RF_Random65_g1089));
				float4 appendResult99_g1089 = (float4(temp_output_83_0_g1089 , appendResult88_g1089 , 0.0));
				float4 break104_g1089 = appendResult99_g1089;
				float lerpResult117_g1089 = lerp( break104_g1089.y , break104_g1089.z , pow( simplePerlin2D46_g1089 , 2.0 ));
				float RF_Timespeed77_g1089 = _DripLineSpeed;
				float mulTime105_g1089 = _TimeParameters.x * -RF_Timespeed77_g1089;
				float4 XProjection98_g1089 = appendResult16_g1089;
				float3 temp_output_106_0_g1089 = ( float3(0,-1,0) * _TilingVertical );
				float4 tex2DNode147_g1089 = tex2D( _GradientTexRain1, ( ( lerpResult117_g1089 * ( mulTime105_g1089 + pow( simplePerlin2D46_g1089 , 2.0 ) ) ) + ( XProjection98_g1089 * float4( temp_output_106_0_g1089 , 0.0 ) ) ).xy );
				float4 temp_cast_16 = (tex2DNode147_g1089.r).xxxx;
				float4 temp_cast_17 = (MaskWorldX585).xxxx;
				float4 InputMaskWorldX199_g1089 = temp_cast_17;
				float4 temp_output_168_0_g1089 = ( CalculateContrast(clampResult243_g1089,temp_cast_16) * InputMaskWorldX199_g1089 * clampResult243_g1089 );
				float4 clampResult173_g1089 = clamp( temp_output_168_0_g1089 , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float clampResult156_g1089 = clamp( (-1.0 + (_TrailDropContrast - 0.0) * (1.0 - -1.0) / (1.0 - 0.0)) , -1.0 , 100.0 );
				float mulTime40_g1089 = _TimeParameters.x * _TrailDropSpeed;
				float time48_g1089 = mulTime40_g1089;
				float2 voronoiSmoothId48_g1089 = 0;
				float2 texCoord41_g1089 = packedInput.ase_texcoord5.xyz.xy * float2( 1,1 ) + float2( 0,0 );
				float2 coords48_g1089 = texCoord41_g1089 * _TrailDropTiling;
				float2 id48_g1089 = 0;
				float2 uv48_g1089 = 0;
				float fade48_g1089 = 0.5;
				float voroi48_g1089 = 0;
				float rest48_g1089 = 0;
				for( int it48_g1089 = 0; it48_g1089 <2; it48_g1089++ ){
				voroi48_g1089 += fade48_g1089 * voronoi48_g1089( coords48_g1089, time48_g1089, id48_g1089, uv48_g1089, 0,voronoiSmoothId48_g1089 );
				rest48_g1089 += fade48_g1089;
				coords48_g1089 *= 2;
				fade48_g1089 *= 0.5;
				}//Voronoi48_g1089
				voroi48_g1089 /= rest48_g1089;
				float temp_output_52_0_g1089 = ( voroi48_g1089 * (2.0 + (_TrailDropSize - 0.0) * (4.0 - 2.0) / (1.0 - 0.0)) );
				float temp_output_72_0_g1089 = ( step( voroi48_g1089 , temp_output_52_0_g1089 ) * ( 1.0 - temp_output_52_0_g1089 ) );
				float blendOpSrc112_g1089 = tex2D( _GradientTexRain1, ( XProjectionAmount23_g1089 / 8.0 ).xy ).a;
				float blendOpDest112_g1089 = ( tex2D( _GradientTexRain1, ( XProjectionAmount23_g1089 / 8.0 ).xy ).a * temp_output_72_0_g1089 );
				#ifdef _ENABLETRAILOFDROP_ON
				float staticSwitch129_g1089 = ( blendOpSrc112_g1089 + blendOpDest112_g1089 );
				#else
				float staticSwitch129_g1089 = tex2D( _GradientTexRain1, ( XProjectionAmount23_g1089 / 8.0 ).xy ).a;
				#endif
				float4 XTrailOfDrop161_g1089 = saturate( ( staticSwitch129_g1089 * _IntensityDripLine * InputMaskWorldX199_g1089 ) );
				float4 blendOpSrc177_g1089 = clampResult173_g1089;
				float4 blendOpDest177_g1089 = ( CalculateContrast(clampResult156_g1089,( tex2DNode147_g1089.b * InputMaskWorldX199_g1089 )) * XTrailOfDrop161_g1089 );
				#ifdef _ENABLETRAILOFDROP_ON
				float4 staticSwitch235_g1089 = ( blendOpSrc177_g1089 + blendOpDest177_g1089 );
				#else
				float4 staticSwitch235_g1089 = clampResult173_g1089;
				#endif
				float4 appendResult17_g1089 = (float4(worldToObj249_g1089.x , worldToObj249_g1089.y , 0.0 , 0.0));
				float4 ZProjection97_g1089 = appendResult17_g1089;
				float4 ZProjectionAmount22_g1089 = ( appendResult17_g1089 * ( ( _Globaltiling.y * _TilingHorizontal ) * clampResult5_g1089 ) );
				float simplePerlin2D49_g1089 = snoise( ( floor( ZProjectionAmount22_g1089 ) * float4( (( _EnableProjectionGrid )?( float2( 1,1 ) ):( float2( 1,0 ) )), 0.0 , 0.0 ) ).xy*( RandomPosition25_g1089 + 0.5 ) );
				simplePerlin2D49_g1089 = simplePerlin2D49_g1089*0.5 + 0.5;
				float temp_output_80_0_g1089 = round( ( pow( simplePerlin2D49_g1089 , 2.0 ) * temp_output_240_0_g1089 ) );
				float2 appendResult85_g1089 = (float2(RF_RandomMin66_g1089 , RF_Random65_g1089));
				float4 appendResult101_g1089 = (float4(temp_output_80_0_g1089 , appendResult85_g1089 , 0.0));
				float4 break110_g1089 = appendResult101_g1089;
				float lerpResult121_g1089 = lerp( break110_g1089.y , break110_g1089.z , pow( simplePerlin2D49_g1089 , 2.0 ));
				float mulTime109_g1089 = _TimeParameters.x * -RF_Timespeed77_g1089;
				float4 tex2DNode152_g1089 = tex2D( _GradientTexRain1, ( ( float4( temp_output_106_0_g1089 , 0.0 ) * ZProjection97_g1089 ) + ( lerpResult121_g1089 * ( mulTime109_g1089 + pow( simplePerlin2D49_g1089 , 2.0 ) ) ) ).xy );
				float4 temp_cast_28 = (tex2DNode152_g1089.r).xxxx;
				float4 temp_cast_29 = (MaskWorldZ583).xxxx;
				float4 InputMaskWorldZ205_g1089 = temp_cast_29;
				float4 clampResult175_g1089 = clamp( ( CalculateContrast(clampResult243_g1089,temp_cast_28) * InputMaskWorldZ205_g1089 * clampResult243_g1089 ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float blendOpSrc118_g1089 = tex2D( _GradientTexRain1, ( ZProjectionAmount22_g1089 / 8.0 ).xy ).a;
				float blendOpDest118_g1089 = ( tex2D( _GradientTexRain1, ( ZProjectionAmount22_g1089 / 8.0 ).xy ).a * temp_output_72_0_g1089 );
				#ifdef _ENABLETRAILOFDROP_ON
				float staticSwitch124_g1089 = ( blendOpSrc118_g1089 + blendOpDest118_g1089 );
				#else
				float staticSwitch124_g1089 = tex2D( _GradientTexRain1, ( ZProjectionAmount22_g1089 / 8.0 ).xy ).a;
				#endif
				float4 ZTrailOfDrop157_g1089 = saturate( ( staticSwitch124_g1089 * _IntensityDripLine * InputMaskWorldZ205_g1089 ) );
				float4 blendOpSrc179_g1089 = clampResult175_g1089;
				float4 blendOpDest179_g1089 = ( CalculateContrast(clampResult156_g1089,( tex2DNode152_g1089.b * InputMaskWorldZ205_g1089 )) * ZTrailOfDrop157_g1089 );
				float4 blendOpSrc186_g1089 = staticSwitch235_g1089;
				float4 blendOpDest186_g1089 = ( saturate( ( blendOpSrc179_g1089 + blendOpDest179_g1089 ) ));
				float4 blendOpSrc185_g1089 = saturate( ( tex2D( _GradientTexRain1, ( XProjectionAmount23_g1089 / 8.0 ).xy ).a * _IntensityDripLine * InputMaskWorldX199_g1089 ) );
				float4 blendOpDest185_g1089 = saturate( ( tex2D( _GradientTexRain1, ( ZProjectionAmount22_g1089 / 8.0 ).xy ).a * _IntensityDripLine * InputMaskWorldZ205_g1089 ) );
				float4 blendOpSrc188_g1089 = InputMaskWorldX199_g1089;
				float4 blendOpDest188_g1089 = InputMaskWorldZ205_g1089;
				float4 temp_cast_39 = (MaskWorldY589).xxxx;
				float4 InputMaskWorldY207_g1089 = temp_cast_39;
				float4 blendOpSrc190_g1089 = ( saturate( max( blendOpSrc188_g1089, blendOpDest188_g1089 ) ));
				float4 blendOpDest190_g1089 = ( InputMaskWorldY207_g1089 * 2.0 );
				float4 clampResult192_g1089 = clamp( ( ( ( saturate( ( blendOpSrc186_g1089 + blendOpDest186_g1089 ) )) * ( saturate( max( blendOpSrc185_g1089, blendOpDest185_g1089 ) )) ) * ( saturate( max( blendOpSrc190_g1089, blendOpDest190_g1089 ) )) ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 blendOpSrc226_g1089 = ( temp_output_83_0_g1089 * InputMaskWorldX199_g1089 );
				float4 blendOpDest226_g1089 = ( temp_output_80_0_g1089 * InputMaskWorldZ205_g1089 );
				float4 temp_output_226_0_g1089 = ( saturate( max( blendOpSrc226_g1089, blendOpDest226_g1089 ) ));
				float4 MaskRandom231_g1089 = ( temp_output_226_0_g1089 + temp_output_226_0_g1089 );
				float4 RainDropsFalling766 = (( _EnableRandomMask )?( ( (( _UseBaseColorAlphaMaskSide1 )?( ( clampResult192_g1089 * BaseColorAlpha46 ) ):( clampResult192_g1089 )) * MaskRandom231_g1089 ) ):( (( _UseBaseColorAlphaMaskSide1 )?( ( clampResult192_g1089 * BaseColorAlpha46 ) ):( clampResult192_g1089 )) ));
				float4 blendOpSrc772 = temp_cast_8;
				float4 blendOpDest772 = RainDropsFalling766;
				float height107_g1092 = ( saturate( max( blendOpSrc772, blendOpDest772 ) )).r;
				float scale107_g1092 = ( _NormalDrop / 10.0 );
				float3 localPerturbNormal107_g1092 = PerturbNormal107_g1092( surf_pos107_g1092 , surf_norm107_g1092 , height107_g1092 , scale107_g1092 );
				float3x3 ase_worldToTangent = float3x3(tangentWS.xyz,ase_worldBitangent,normalWS);
				float3 worldToTangentDir42_g1092 = mul( ase_worldToTangent, localPerturbNormal107_g1092);
				float2 uv_BumpMap = packedInput.ase_texcoord5.xyz.xy * _BumpMap_ST.xy + _BumpMap_ST.zw;
				float2 break26_g1091 = uv_BumpMap;
				float2 appendResult14_g1091 = (float2(( break26_g1091.x * GlobalTilingX11 ) , ( break26_g1091.y * GlobalTilingY8 )));
				float2 appendResult13_g1091 = (float2(( break26_g1091.x + GlobalOffsetX10 ) , ( break26_g1091.y + GlobalOffsetY9 )));
				float3 unpack4_g1090 = UnpackNormalScale( tex2D( _BumpMap, ( ( appendResult14_g1091 + appendResult13_g1091 ) + Parallaxe494 ) ), _BumpScale );
				unpack4_g1090.z = lerp( 1, unpack4_g1090.z, saturate(_BumpScale) );
				float3 Normal27 = BlendNormal( worldToTangentDir42_g1092 , unpack4_g1090 );
				...
				float4 fetchOpaqueVal13_g1093 = ASEHDSampleSceneColor(( (ase_grabScreenPosNorm).xyzw + (( ( float4( Normal27 , 0.0 ) + float4( mul( float4( normalizedWorldNormal , 0.0 ), UNITY_MATRIX_V ).xyz , 0.0 ) ) * (-1.0 + (_Refraction - 0.0) * (1.0 - -1.0) / (2.0 - 0.0)) )).xyzw ).xy, 0.0, GetInverseCurrentExposureMultiplier());
				float2 uv_TransparentMask = packedInput.ase_texcoord5.xyz.xy * _TransparentMask_ST.xy + _TransparentMask_ST.zw;
				float4 temp_cast_48 = (( (( _InvertTransparent1 )?( ( 1.0 - (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) ) ):( (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) )) + (-1.2 + (_SpreadTransparentMap1 - 0.0) * (0.7 - -1.2) / (1.0 - 0.0)) )).xxxx;
				float4 clampResult34_g1093 = clamp( CalculateContrast(( _ContrastTransparentMap1 + 1.0 ),temp_cast_48) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 GrabSreenRefraction892 = ( ( fetchOpaqueVal13_g1093 * clampResult34_g1093 ) * _OpacityGrabPass );
				float2 uv_EmissionMap = packedInput.ase_texcoord5.xyz.xy * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
				float2 break26_g1095 = uv_EmissionMap;
				float2 appendResult14_g1095 = (float2(( break26_g1095.x * GlobalTilingX11 ) , ( break26_g1095.y * GlobalTilingY8 )));
				float2 appendResult13_g1095 = (float2(( break26_g1095.x + GlobalOffsetX10 ) , ( break26_g1095.y + GlobalOffsetY9 )));
				...
				float4 temp_cast_51 = (RainDrops606).xxxx;
				float4 blendOpSrc780 = RainDropsFalling766;
				float4 blendOpDest780 = temp_cast_51;
				float4 temp_output_780_0 = ( saturate( max( blendOpSrc780, blendOpDest780 ) ));
				float4 temp_cast_52 = (RainDrops606).xxxx;
				#if defined(_REFLECTED_ALL)
				float4 staticSwitch963 = temp_cast_49;
				#elif defined(_REFLECTED_RAINDROPS)
				float4 staticSwitch963 = temp_output_780_0;
				#elif defined(_REFLECTED_DROPONLY)
				float4 staticSwitch963 = temp_cast_52;
				#elif defined(_REFLECTED_DRIPLINEONLY)
				float4 staticSwitch963 = RainDropsFalling766;
				#else
				float4 staticSwitch963 = temp_cast_49;
				#endif
				float4 Emission110 = ( GrabSreenRefraction892 + ( _EmissionColor * tex2D( _EmissionMap, ( ( appendResult14_g1095 + appendResult13_g1095 ) + Parallaxe494 ) ) * _EmissionIntensity ) + ( ( texCUBENode27_g1096 * ( texCUBENode27_g1096.a * clampResult58_g1096 ) * _ColorCubemap1 ) * staticSwitch963 ) );
				
				float temp_output_1_0_g621 = ( tex2DNode3_g621.a * _Glossiness );
				float4 temp_cast_56 = (RainDrops606).xxxx;
				float4 blendOpSrc707 = temp_cast_56;
				...
				float4 temp_cast_57 = (temp_output_1_0_g621).xxxx;
				float4 blendOpSrc613 = temp_cast_57;
				...
				float4 lerpResult46_g1093 = lerp( ( 1.0 - clampResult34_g1093 ) , float4( 1,1,1,0 ) , ( 1.0 - _FadeOpacity ));
				float4 Fade922 = lerpResult46_g1093;
				...
				surfaceDescription.Alpha = Fade922.r;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float3 surf_pos107_g1092 = ase_worldPos;
				float3 surf_norm107_g1092 = ase_worldNormal;
				float2 temp_cast_3 = (_SplashSpeed).xx;
				float time58_g999 = ( 1.0 * 0.001 );
				float2 voronoiSmoothId58_g999 = 0;
				float2 texCoord55_g999 = packedInput.ase_texcoord.xyz.xy * float2( 1,1 ) + float2( 0,0 );
				float temp_output_577_0 = ( _NoiseScale * 100.0 );
				float clampResult962 = clamp( temp_output_577_0 , 0.0 , 10000.0 );
				float time877 = 0.0;
				float2 voronoiSmoothId877 = 0;
				...
				float time48_g1089 = mulTime40_g1089;
				float2 voronoiSmoothId48_g1089 = 0;
				float2 texCoord41_g1089 = packedInput.ase_texcoord.xyz.xy * float2( 1,1 ) + float2( 0,0 );
				float2 coords48_g1089 = texCoord41_g1089 * _TrailDropTiling;
				float2 id48_g1089 = 0;
				float2 uv48_g1089 = 0;
				float fade48_g1089 = 0.5;
				...
				float4 fetchOpaqueVal13_g1093 = ASEHDSampleSceneColor(( (ase_grabScreenPosNorm).xyzw + (( ( float4( Normal27 , 0.0 ) + float4( mul( float4( normalizedWorldNormal , 0.0 ), UNITY_MATRIX_V ).xyz , 0.0 ) ) * (-1.0 + (_Refraction - 0.0) * (1.0 - -1.0) / (2.0 - 0.0)) )).xyzw ).xy, 0.0, GetInverseCurrentExposureMultiplier());
				float2 uv_TransparentMask = packedInput.ase_texcoord.xyz.xy * _TransparentMask_ST.xy + _TransparentMask_ST.zw;
				float4 temp_cast_48 = (( (( _InvertTransparent1 )?( ( 1.0 - (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) ) ):( (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) )) + (-1.2 + (_SpreadTransparentMap1 - 0.0) * (0.7 - -1.2) / (1.0 - 0.0)) )).xxxx;
				float4 clampResult34_g1093 = clamp( CalculateContrast(( _ContrastTransparentMap1 + 1.0 ),temp_cast_48) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 GrabSreenRefraction892 = ( ( fetchOpaqueVal13_g1093 * clampResult34_g1093 ) * _OpacityGrabPass );
		...
		Pass
		{
					...
					)
			{
				...
				float2 uv_TransparentMask = packedInput.ase_texcoord1.xy * _TransparentMask_ST.xy + _TransparentMask_ST.zw;
				...
				float GlobalOffsetY9 = _GlobalXYTilingXYZWOffsetXY.w;
				float2 appendResult13_g818 = (float2(( break26_g818.x + GlobalOffsetX10 ) , ( break26_g818.y + GlobalOffsetY9 )));
				float2 uv_MetallicGlossMapMAHS = packedInput.ase_texcoord1.xy * _MetallicGlossMapMAHS_ST.xy + _MetallicGlossMapMAHS_ST.zw;
				float2 break26_g622 = uv_MetallicGlossMapMAHS;
				float2 appendResult14_g622 = (float2(( break26_g622.x * GlobalTilingX11 ) , ( break26_g622.y * GlobalTilingY8 )));
				float2 appendResult13_g622 = (float2(( break26_g622.x + GlobalOffsetX10 ) , ( break26_g622.y + GlobalOffsetY9 )));
				float4 tex2DNode3_g621 = tex2D( _MetallicGlossMapMAHS, ( ( appendResult14_g622 + appendResult13_g622 ) + float2( 0,0 ) ) );
				...
				float4 tex2DNode7_g817 = tex2D( _MainTex, ( ( appendResult14_g818 + appendResult13_g818 ) + Parallaxe494 ) );
				float BaseColorAlpha46 = (( _InvertABaseColor )?( ( 1.0 - tex2DNode7_g817.a ) ):( tex2DNode7_g817.a ));
				float4 temp_cast_0 = (( (( _InvertTransparent1 )?( ( 1.0 - (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) ) ):( (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) )) + (-1.2 + (_SpreadTransparentMap1 - 0.0) * (0.7 - -1.2) / (1.0 - 0.0)) )).xxxx;
				float4 clampResult34_g1093 = clamp( CalculateContrast(( _ContrastTransparentMap1 + 1.0 ),temp_cast_0) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 lerpResult46_g1093 = lerp( ( 1.0 - clampResult34_g1093 ) , float4( 1,1,1,0 ) , ( 1.0 - _FadeOpacity ));
				float4 Fade922 = lerpResult46_g1093;
		...
		Pass
		{
					...
					)
			{
				...
				float time58_g999 = ( 1.0 * 0.001 );
				float2 voronoiSmoothId58_g999 = 0;
				float2 texCoord55_g999 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float temp_output_577_0 = ( _NoiseScale * 100.0 );
				float clampResult962 = clamp( temp_output_577_0 , 0.0 , 10000.0 );
				float time877 = 0.0;
				float2 voronoiSmoothId877 = 0;
				...
				float2 coords58_g999 = ( float4( texCoord55_g999, 0.0 , 0.0 ) + temp_cast_2 ).xy * _Tiling;
				float2 id58_g999 = 0;
				float2 uv58_g999 = 0;
				float fade58_g999 = 0.5;
				float voroi58_g999 = 0;
				...
				voroi58_g999 /= rest58_g999;
				float2 myVarName60_g999 = id58_g999;
				float2 panner63_g999 = ( 1.0 * _Time.y * temp_cast_0 + ( step( voroi58_g999 , 0.1 ) * myVarName60_g999 ));
				float temp_output_79_0_g999 = (0.0 + (_Size - 0.0) * (0.05 - 0.0) / (1.0 - 0.0));
				float temp_output_120_0_g999 = saturate( ( step( voroi58_g999 , temp_output_79_0_g999 ) * ( 1.0 - ( voroi58_g999 * temp_output_79_0_g999 * (1.0 + (_DotFalloff - 0.0) * (500.0 - 1.0) / (1.0 - 0.0)) ) ) ) );
				float MaskWorldY589 = normalWS.y;
				float temp_output_667_0 = ( MaskWorldY589 * _DropTopIntensity );
				...
				float3 temp_output_106_0_g1089 = ( float3(0,-1,0) * _TilingVertical );
				float4 tex2DNode147_g1089 = tex2D( _GradientTexRain1, ( ( lerpResult117_g1089 * ( mulTime105_g1089 + pow( simplePerlin2D46_g1089 , 2.0 ) ) ) + ( XProjection98_g1089 * float4( temp_output_106_0_g1089 , 0.0 ) ) ).xy );
				float4 temp_cast_13 = (tex2DNode147_g1089.r).xxxx;
				float4 temp_cast_14 = (MaskWorldX585).xxxx;
				float4 InputMaskWorldX199_g1089 = temp_cast_14;
				float4 temp_output_168_0_g1089 = ( CalculateContrast(clampResult243_g1089,temp_cast_13) * InputMaskWorldX199_g1089 * clampResult243_g1089 );
				float4 clampResult173_g1089 = clamp( temp_output_168_0_g1089 , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float clampResult156_g1089 = clamp( (-1.0 + (_TrailDropContrast - 0.0) * (1.0 - -1.0) / (1.0 - 0.0)) , -1.0 , 100.0 );
				float mulTime40_g1089 = _TimeParameters.x * _TrailDropSpeed;
				float time48_g1089 = mulTime40_g1089;
				float2 voronoiSmoothId48_g1089 = 0;
				float2 texCoord41_g1089 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float2 coords48_g1089 = texCoord41_g1089 * _TrailDropTiling;
				float2 id48_g1089 = 0;
				float2 uv48_g1089 = 0;
				float fade48_g1089 = 0.5;
				...
				float mulTime109_g1089 = _TimeParameters.x * -RF_Timespeed77_g1089;
				float4 tex2DNode152_g1089 = tex2D( _GradientTexRain1, ( ( float4( temp_output_106_0_g1089 , 0.0 ) * ZProjection97_g1089 ) + ( lerpResult121_g1089 * ( mulTime109_g1089 + pow( simplePerlin2D49_g1089 , 2.0 ) ) ) ).xy );
				float4 temp_cast_25 = (tex2DNode152_g1089.r).xxxx;
				float4 temp_cast_26 = (MaskWorldZ583).xxxx;
				float4 InputMaskWorldZ205_g1089 = temp_cast_26;
				float4 clampResult175_g1089 = clamp( ( CalculateContrast(clampResult243_g1089,temp_cast_25) * InputMaskWorldZ205_g1089 * clampResult243_g1089 ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float blendOpSrc118_g1089 = tex2D( _GradientTexRain1, ( ZProjectionAmount22_g1089 / 8.0 ).xy ).a;
				float blendOpDest118_g1089 = ( tex2D( _GradientTexRain1, ( ZProjectionAmount22_g1089 / 8.0 ).xy ).a * temp_output_72_0_g1089 );
				#ifdef _ENABLETRAILOFDROP_ON
				float staticSwitch124_g1089 = ( blendOpSrc118_g1089 + blendOpDest118_g1089 );
				...
				float4 InputMaskWorldY207_g1089 = temp_cast_36;
				float4 blendOpSrc190_g1089 = ( saturate( max( blendOpSrc188_g1089, blendOpDest188_g1089 ) ));
				float4 blendOpDest190_g1089 = ( InputMaskWorldY207_g1089 * 2.0 );
				float4 clampResult192_g1089 = clamp( ( ( ( saturate( ( blendOpSrc186_g1089 + blendOpDest186_g1089 ) )) * ( saturate( max( blendOpSrc185_g1089, blendOpDest185_g1089 ) )) ) * ( saturate( max( blendOpSrc190_g1089, blendOpDest190_g1089 ) )) ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 blendOpSrc226_g1089 = ( temp_output_83_0_g1089 * InputMaskWorldX199_g1089 );
				...
				float2 uv_TransparentMask = packedInput.ase_texcoord3.xy * _TransparentMask_ST.xy + _TransparentMask_ST.zw;
				float4 temp_cast_43 = (( (( _InvertTransparent1 )?( ( 1.0 - (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) ) ):( (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) )) + (-1.2 + (_SpreadTransparentMap1 - 0.0) * (0.7 - -1.2) / (1.0 - 0.0)) )).xxxx;
				float4 clampResult34_g1093 = clamp( CalculateContrast(( _ContrastTransparentMap1 + 1.0 ),temp_cast_43) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 lerpResult46_g1093 = lerp( ( 1.0 - clampResult34_g1093 ) , float4( 1,1,1,0 ) , ( 1.0 - _FadeOpacity ));
				float4 Fade922 = lerpResult46_g1093;
		...
		Pass
		{
				...
				)
			{
				...
				float time58_g999 = ( 1.0 * 0.001 );
				float2 voronoiSmoothId58_g999 = 0;
				float2 texCoord55_g999 = packedInput.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float temp_output_577_0 = ( _NoiseScale * 100.0 );
				float clampResult962 = clamp( temp_output_577_0 , 0.0 , 10000.0 );
				float time877 = 0.0;
				float2 voronoiSmoothId877 = 0;
				...
				float time48_g1089 = mulTime40_g1089;
				float2 voronoiSmoothId48_g1089 = 0;
				float2 texCoord41_g1089 = packedInput.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float2 coords48_g1089 = texCoord41_g1089 * _TrailDropTiling;
				float2 id48_g1089 = 0;
				float2 uv48_g1089 = 0;
				float fade48_g1089 = 0.5;
				...
				float2 uv_TransparentMask = packedInput.ase_texcoord5.xy * _TransparentMask_ST.xy + _TransparentMask_ST.zw;
				float4 temp_cast_43 = (( (( _InvertTransparent1 )?( ( 1.0 - (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) ) ):( (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) )) + (-1.2 + (_SpreadTransparentMap1 - 0.0) * (0.7 - -1.2) / (1.0 - 0.0)) )).xxxx;
				float4 clampResult34_g1093 = clamp( CalculateContrast(( _ContrastTransparentMap1 + 1.0 ),temp_cast_43) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 lerpResult46_g1093 = lerp( ( 1.0 - clampResult34_g1093 ) , float4( 1,1,1,0 ) , ( 1.0 - _FadeOpacity ));
				float4 Fade922 = lerpResult46_g1093;
		...
		Pass
		{
						...
						)
			{
				...
				float time58_g999 = ( 1.0 * 0.001 );
				float2 voronoiSmoothId58_g999 = 0;
				float2 texCoord55_g999 = packedInput.ase_texcoord7.xyz.xy * float2( 1,1 ) + float2( 0,0 );
				float temp_output_577_0 = ( _NoiseScale * 100.0 );
				float clampResult962 = clamp( temp_output_577_0 , 0.0 , 10000.0 );
				float time877 = 0.0;
				float2 voronoiSmoothId877 = 0;
				...
				float time48_g1089 = mulTime40_g1089;
				float2 voronoiSmoothId48_g1089 = 0;
				float2 texCoord41_g1089 = packedInput.ase_texcoord7.xyz.xy * float2( 1,1 ) + float2( 0,0 );
				float2 coords48_g1089 = texCoord41_g1089 * _TrailDropTiling;
				float2 id48_g1089 = 0;
				float2 uv48_g1089 = 0;
				float fade48_g1089 = 0.5;
				...
				float3 normalizedWorldNormal = normalize( normalWS );
				float4 fetchOpaqueVal13_g1093 = ASEHDSampleSceneColor(( (ase_grabScreenPosNorm).xyzw + (( ( float4( Normal27 , 0.0 ) + float4( mul( float4( normalizedWorldNormal , 0.0 ), UNITY_MATRIX_V ).xyz , 0.0 ) ) * (-1.0 + (_Refraction - 0.0) * (1.0 - -1.0) / (2.0 - 0.0)) )).xyzw ).xy, 0.0, GetInverseCurrentExposureMultiplier());
				float2 uv_TransparentMask = packedInput.ase_texcoord7.xyz.xy * _TransparentMask_ST.xy + _TransparentMask_ST.zw;
				float4 temp_cast_48 = (( (( _InvertTransparent1 )?( ( 1.0 - (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) ) ):( (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( tex2D( _TransparentMask, uv_TransparentMask ).r )) )) + (-1.2 + (_SpreadTransparentMap1 - 0.0) * (0.7 - -1.2) / (1.0 - 0.0)) )).xxxx;
				float4 clampResult34_g1093 = clamp( CalculateContrast(( _ContrastTransparentMap1 + 1.0 ),temp_cast_48) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 GrabSreenRefraction892 = ( ( fetchOpaqueVal13_g1093 * clampResult34_g1093 ) * _OpacityGrabPass );
